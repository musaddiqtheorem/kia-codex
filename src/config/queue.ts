...
type IngestionJobData = { uploadId: string; sourceType: string; fileName: string };

const processIngestion = async (jobData: IngestionJobData) => {
  const { uploadId, sourceType, fileName } = jobData;
...
if (env.QUEUE_MODE === 'redis') {
  ...
  aiQueue = new Queue('ai-insights', { connection });

  const ingestionWorker = new Worker('ingestion', (job) => processIngestion(job.data as IngestionJobData), { connection });
...
export const enqueueIngestionJob = async (jobData: IngestionJobData) => {
  if (env.QUEUE_MODE === 'memory') {
    await processIngestion(jobData);
    return;
  }

  await ingestionQueue?.add('ingest-file', jobData);
};
...
export const startWorkers = () => {
  // Workers are initialized lazily when QUEUE_MODE=redis.
};
